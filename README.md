...
graph TD
    ...
    %% 품질 검사 흐름
    C --> C1[Home 위치로 이동 및 Top 뷰 확인]
    C1 --> C2[4방향 3D 뷰포인트 생성 및 촬영]
    C2 --> C3[YOLOv8 바운딩 박스 검출]
    C3 --> C4[3D 좌표 변환 및 좌우 보정 매핑]
    C4 --> C5[패턴 매칭을 통한 양품/불량 판정]
    C5 --> HMI[HMI 대시보드 화면 업데이트]
...
- **마이크 (Microphone):** USB 외장 마이크 (음성 인식용)
- **기타:** 시리얼 통신 제어 컨베이어 벨트
