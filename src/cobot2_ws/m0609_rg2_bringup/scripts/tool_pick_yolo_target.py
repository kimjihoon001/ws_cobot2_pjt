...
class PickYoloTarget(Node):
    ...
    def pick_detected_tool(self, detection, delivery_joints_deg) -> bool:
        ...
        self.get_logger().info(
            f'파지 하강 목표 z: detected_base_z={z:.4f}m, '
        ...
        if grasp_refined is not None:
            grasp_refined_x, grasp_refined_y, grasp_refined_z, _, _ = grasp_refined
            refined_grasp_target_z = grasp_refined_z + grasp_z_clearance
            z_adjust = refined_grasp_target_z - grasp_target_z
            self.get_logger().info(
                f'파지직전 z 재보정 확인: measured_base_z={grasp_refined_z:.4f}m, '
                f'current_target_z={grasp_target_z:.4f}m, '
                f'refined_target_z={refined_grasp_target_z:.4f}m, '
                f'delta={z_adjust:+.4f}m, '
                f'x_delta={grasp_refined_x - x:+.4f}m, y_delta={grasp_refined_y - y:+.4f}m'
            )

            if abs(z_adjust) > GRASP_REFINE_MAX_Z_ADJUST:
                self.get_logger().warn(
                    f'파지직전 z 보정량이 너무 큼({z_adjust:+.4f}m) - 안전상 추가 이동 생략'
                )
            elif abs(z_adjust) < GRASP_REFINE_MIN_Z_ADJUST:
                self.get_logger().info(
                    f'파지직전 z 보정량이 작음({z_adjust:+.4f}m) - 추가 이동 생략'
                )
            elif self.move_cartesian_grasp(
                x, y, refined_grasp_target_z, yaw, seed_joints=pregrasp_joints
            ):
                z = grasp_refined_z
                grasp_target_z = refined_grasp_target_z
                world_position, world_quat, gripper_position, gripper_quat = compute_attach_pose(
                    **attach_pose_params,
                    axis_angle=axis_angle,
                    yaw=yaw,
                ...
                self.get_logger().info(
                    f'파지직전 z 재보정 이동 완료: target_z={grasp_target_z:.4f}m'
                )
            else:
                self.get_logger().warn('파지직전 z 재보정 이동 실패 - 현재 위치에서 파지 진행')
