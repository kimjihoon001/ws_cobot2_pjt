...
class ToolObstaclePublisher(Node):
    def __init__(self):
        super().__init__('tool_obstacle_publisher')

        # 1. Parameter declaration
        self.declare_parameter('target_tool', 'screwdriver')  # drill, hammer, pliers, screwdriver, wrench
        ...
        # 2. Service Client to query tool 3D position
        self.client = self.create_client(SrvDepthPosition, 'get_3d_position')
        self.get_logger().info(f"Waiting for 'get_3d_position' service...")
        while not self.client.wait_for_service(timeout_sec=2.0):
            self.get_logger().info("Service 'get_3d_position' not available, waiting...")
        ...
        self.get_logger().info(f"ToolObstaclePublisher initialized. Target: {self.target_tool} at {rate} Hz (Persistence: {self.clear_timeout}s).")
        self.is_busy = False
