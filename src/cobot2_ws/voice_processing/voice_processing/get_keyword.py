# ros2 service call /get_keyword std_srvs/srv/Trigger "{}"

import os
import rclpy
import pyaudio
import subprocess
from rclpy.node import Node

from ament_index_python.packages import get_package_share_directory
from dotenv import load_dotenv
from langchain_openai import ChatOpenAI
from langchain.prompts import PromptTemplate  # d2 이거를 langchain_core로 바꿈
# from langchain.chains import LLMChain

from std_srvs.srv import Trigger
from od_msg.srv import ConfirmTools
from voice_processing.MicController import MicController, MicConfig

from voice_processing.wakeup_word import WakeupWord
from voice_processing.stt import STT

############ Package Path & Environment Setting ############

#----------------------------------------------------------------
# current_dir = os.getcwd()
# package_path = get_package_share_directory("pick_and_place_voice")

# env_path = "/home/rokey/cobot_ws/src/cobot2_ws/pick_and_place_voice/resource/.env"
# load_dotenv(dotenv_path=env_path)
# is_load = load_dotenv(dotenv_path=os.path.join(f"{package_path}/resource/.env"))
# openai_api_key = os.getenv("OPENAI_API_KEY")
#-----------------------------------------------------------------

PACKAGE_NAME = "voice_processing"
PACKAGE_PATH = get_package_share_directory(PACKAGE_NAME)
RESOURCE_PATH = os.path.join(PACKAGE_PATH, "resource")
ENV_PATH = os.path.join(RESOURCE_PATH, ".env")
load_dotenv(dotenv_path=ENV_PATH)
openai_api_key = os.getenv("OPENAI_API_KEY")

# 제품별로 필요한 도구 매핑 (LLM이 아니라 코드가 확실하게 처리하기 위한 딕셔너리)
PRODUCT_TOOL_MAP = {
    "product1": ["hammer", "screwdriver"],
    "product2": ["hammer", "screwdriver"],
    "product3": ["hammer", "screwdriver"],
}

############ AI Processor ############
# class AIProcessor:
#     def __init__(self):



############ GetKeyword Node ############
class GetKeyword(Node):
    def __init__(self):

        print(PACKAGE_PATH, RESOURCE_PATH, ENV_PATH)

        self.llm = ChatOpenAI(
            model="gpt-4o", temperature=0.5, openai_api_key=openai_api_key
        )

        prompt_content = """
            당신은 로봇에게 명령을 전달하기 위해, 사용자의 음성 명령에서 도구와 목적지만 정확히 추출하는 파서(parser)입니다.
            인사말, 설명, 부연 설명 없이 오직 지정된 출력 형식으로만 답하세요.

            <목표>
            - 문장에서 다음 리스트에 포함된 도구를 최대한 정확히 추출하세요.
            - 문장에 등장하는 도구의 목적지(어디로 옮기라고 했는지)도 함께 추출하세요.

            <도구 리스트>
            - hammer, screwdriver, pos1, pos2, pos3, user

            <제품별 필요 도구 매핑>
            - product1: hammer, screwdriver
            (테스트용으로 우선 product1만 정의되어 있습니다. 목록에 없는 product는 매핑하지 말고 도구 없음으로 처리하세요.)

            <출력 형식>
            - 다음 형식을 반드시 따르세요: [도구1 도구2 ... / pos1 pos2 ...]
            - 도구와 위치는 각각 공백으로 구분
            - 도구가 없으면 앞쪽은 공백 없이 비우고, 목적지가 없으면 '/' 뒤는 공백 없이 비웁니다.
            - 도구와 목적지의 순서는 등장 순서를 따릅니다.

            <특수 규칙>
            - 명확한 도구 명칭이 없지만 문맥상 유추 가능한 경우(예: "망치", "못 박는 것" → hammer / "나사 돌리는 것", "드라이버" → screwdriver)는 리스트 내 항목으로 최대한 추론해 반환하세요.
            - 다수의 도구와 목적지가 동시에 등장할 경우 각각에 대해 정확히 매칭하여 순서대로 출력하세요.
            - 사용자가 개별 도구 이름 대신 "product1"처럼 제품명을 말하며 도구를 가져오라고 하면,
              <제품별 필요 도구 매핑>에서 그 제품에 해당하는 도구를 전부 찾아 출력하세요.
            - 이때 목적지를 따로 말하지 않았다면, 전개된 도구 전부의 목적지를 동일하게 user로 채우세요.
              (사용자가 목적지를 명시하면 그 목적지를 그대로 사용하세요.)

            <예시>
            - 입력: "hammer를 user에게 가져다 놔"
            출력: hammer / user

            - 입력: "왼쪽에 있는 망치를 작업대에 넣어줘"
            출력: hammer / user

            - 입력: "screwdriver 좀 pos1에 놔줘"
            출력: screwdriver / pos1

            - 입력: "나사 돌리는 거 좀 pos2에 놔줘"
            출력: screwdriver / pos2

            - 입력: "hammer는 pos2에 두고 screwdriver는 pos1에 둬"
            출력: hammer screwdriver / pos2 pos1

            - 입력: "pos3로 옮겨줘"
            출력: / pos3

            - 입력: "screwdriver 좀 줘"
            출력: screwdriver /

            - 입력: "product1 할건데 tool 가져와"
            출력: hammer screwdriver / user user

            - 입력: "product1 작업할 거야, 필요한 도구 좀 pos1에 갖다 놔"
            출력: hammer screwdriver / pos1 pos1

            - 입력: "product2 작업할 거야, 필요한 도구 좀 pos2에 갖다 놔"
            출력: hammer screwdriver / pos2 pos2

            - 입력: "product3 작업할 거야, 필요한 도구 좀 pos3에 갖다 놔"
            출력: hammer screwdriver / pos3 pos3
            
            - 입력: "안녕, 오늘 날씨 좋다"
            출력: /

            <사용자 입력>
            "{user_input}"                
        """

        self.prompt_template = PromptTemplate(
            input_variables=["user_input"], template=prompt_content
        )
        self.lang_chain = self.prompt_template | self.llm
        # self.lang_chain = LLMChain(llm=self.llm, prompt=self.prompt_template)
        self.stt = STT(openai_api_key=openai_api_key)


        super().__init__("get_keyword_node")
        # 오디오 설정
        mic_config = MicConfig(
            chunk=12000,
            rate=48000,
            channels=1,
            record_seconds=5,
            fmt=pyaudio.paInt16,
            device_index=10,
            buffer_size=24000,
        )
        self.mic_controller = MicController(config=mic_config)
        # self.ai_processor = AIProcessor()

        self.get_logger().info("MicRecorderNode initialized.")
        self.get_logger().info("wait for client's request... ('hello rokey' wakeup word active)")
        self.get_keyword_srv = self.create_service(
            Trigger, "get_keyword", self.get_keyword
        )
        # HMI 브릿지(FastAPI 안의 ROS2 노드)한테 "이 도구들 맞아?" 물어보는 클라이언트
        self.confirm_client = self.create_client(ConfirmTools, 'confirm_tools')
        self.wakeup_word = WakeupWord(mic_config.buffer_size)
        self.play_audio("sys_start.mp3")

    def play_audio(self, filename):
        audio_path = os.path.join(RESOURCE_PATH, f"audio/{filename}")
        if os.path.exists(audio_path):
            subprocess.run(["gst-play-1.0", audio_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    def extract_keyword(self, output_message):  # d2 이 함수 일부 수정함
        # LLM한테 맡기기 전에, 코드가 먼저 product 언급 여부를 확인해서
        # 확실한 매핑이 있으면 그걸로 바로 반환 (LLM이 매핑을 잘못 기억할 위험 자체를 없앰)
        for product_name, tools in PRODUCT_TOOL_MAP.items():
            if product_name in output_message:
                targets = ["user"] * len(tools)  # product로 부르면 항상 user한테 가져다 놓음
                print(f"'{product_name}' 감지됨 -> 매핑된 도구로 바로 반환: {tools} / {targets}")
                return tools, targets

        response = self.lang_chain.invoke({"user_input": output_message})
        result = response.content

        object, target = result.strip().split("/")

        object = object.split()
        target = target.split()

        print(f"llm's response: {object} / {target}")
        return object, target

    def _ask_hmi_to_confirm(self, tools, targets):
        """HMI 브릿지(FastAPI 안 ROS2 노드)한테 확인 요청을 보내고, 사람이 화면에서
        답할 때까지 기다린다. 브릿지가 안 떠 있거나 응답이 없으면 안전하게 False."""
        if not self.confirm_client.wait_for_service(timeout_sec=5.0):
            self.get_logger().error("confirm_tools 서비스(HMI 브릿지)가 안 떠 있습니다. 확인 없이 진행은 안 함.")
            return False

        req = ConfirmTools.Request()
        req.tools = tools
        req.targets = targets

        future = self.confirm_client.call_async(req)
        rclpy.spin_until_future_complete(self, future, timeout_sec=65.0)  # 브릿지 쪽 60초 타임아웃보다 살짝 여유

        if future.result() is None:
            self.get_logger().error("confirm_tools 응답 타임아웃")
            return False
        return future.result().confirmed

    def get_keyword(self, request, response):  # 요청과 응답 객체를 받아야 함    # d2 이 함수 일부 수정함
        self.play_audio("sys_wait.mp3")
        try:
            print("open stream")
            self.mic_controller.open_stream()
            self.wakeup_word.set_stream(self.mic_controller.stream)
            print("'hello rokey' 호출 대기 중...")
        except OSError:
            self.get_logger().error("Error: Failed to open audio stream")
            self.get_logger().error("please check your device index")
            return None

        while not self.wakeup_word.is_wakeup():
            pass

        # STT --> Keword Extract --> Embedding
        output_message = self.stt.speech2text()
        tools, targets = self.extract_keyword(output_message)

        while True:
            if not tools:
                # 아무 도구도 못 뽑았으면 확인할 것도 없이 바로 종료
                response.success = False
                response.message = ""
                return response

            self.get_logger().warn(f"Detected tools: {tools} / {targets}")

            # === HMI한테 "이거 맞아?" 물어보고, 화면에서 답할 때까지 여기서 대기 ===
            confirmed = self._ask_hmi_to_confirm(tools, targets)

            if confirmed:
                self.play_audio("order_recv.mp3")
                response.success = True
                # 도구:목적지 쌍으로 실어서 반환 (목적지가 응답에 실린 적이 없었던 부분도 같이 해결)
                response.message = " ".join(f"{t}:{d}" for t, d in zip(tools, targets))
                return response
            else:
                # "아니오"면 웨이크워드부터 다시 하지 않고 STT부터 바로 재시도
                self.get_logger().info("사용자가 아니라고 함, 다시 들음")
                output_message = self.stt.speech2text()
                tools, targets = self.extract_keyword(output_message)


def main():  # d2 메인문 일부 수정
    rclpy.init()
    node = GetKeyword()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()


if __name__ == "__main__":
    main()
