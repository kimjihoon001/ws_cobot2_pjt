...
class GetKeyword(Node):
    def __init__(self):
        ...
        prompt_content = """
            ...
            <목표>
            - 문장에서 다음 도구 목록에 포함된 도구를 최대한 정확히 추출하세요.
            ...
            - 사용자가 개별 도구 이름 대신 "A형"/"B형"/"C형"/"기본형"처럼 제품명을 말하며 도구를 가져오라고 하면,
              <제품별 필요 도구 매핑>에서 그 제품에 해당하는 도구를 전부 찾아 출력하세요. 기본형은 도구가 없으므로
              빈 도구 목록(앞쪽을 공백 없이 비움)으로 출력하세요.
    ...
    def _ask_hmi_to_confirm(self, tools, targets):
        ...
        답하거나 음성으로 "네/아니오"라고 말할 때까지 기다린다 (먼저 들리는 쪽 사용).
        브릿지가 안 떠 있거나 둘 다 응답이 없으면 안전하게 False."""
        ...
        # HMI 응답을 기다리는 동안 먼저 음성으로 "네/아니오"를 짧게 시도해본다.
        # 명확히 들렸으면 그 결과를 바로 쓰고 HMI 응답은 기다리지 않는다.
        heard, voice_confirmed = self.listen_yes_no(timeout_sec=5.0)
        if heard:
            self.get_logger().info(f"음성으로 확인 응답 인식됨: {'예' if voice_confirmed else '아니오'}")
            return voice_confirmed

        rclpy.spin_until_future_complete(self, future, timeout_sec=60.0)  # 위 5초 소비했으니 나머지로 브릿지 쪽 60초 타임아웃 커버
    ...
    def get_keyword(self, request, response):  # 요청과 응답 객체를 받아야 함    # d2 이 함수 일부 수정함
        ...
        while True:
            if not tools:
                if matched_product:
                    # 기본형처럼 "도구가 원래 필요 없는 제품"으로 명시 인식된 경우 -
                    # 확인 절차 없이 바로 정상 완료 처리 (집을 도구가 없음)
                    self.get_logger().info("도구가 필요 없는 제품으로 인식됨 (예: 기본형) - 확인 없이 종료")
                    self.play_audio("no_tool_needed.mp3")
                    response.success = True
                    response.message = ""
                    return response
                # 아무것도 인식 못했으면 확인할 것도 없이 바로 종료
