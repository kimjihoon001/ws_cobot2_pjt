...
class GetKeyword(Node):
    def __init__(self):
        ...
        prompt_content = """
            ...
            <도구 리스트>
            - wood_block, tool, hammer, screwdriver, wrench, pos1, pos2, pos3, user
            ...
            <특수 규칙>
            - 명확한 도구 명칭이 없지만 문맥상 유추 가능한 경우(예: "나무 조각", "블록" → wood_block / "장비", "공구" → tool / "망치" → hammer)는 리스트 내 항목으로 최대한 추론해 반환하세요.
            - 다수의 도구와 목적지가 동시에 등장할 경우 각각에 대해 정확히 매칭하여 순서대로 출력하세요.

            <예시>
            - 입력: "wood_block을 user에게 가져다 놔"  
            출력: wood_block / user

            - 입력: "왼쪽에 있는 나무 블록을 작업대에 넣어줘"  
            출력: wood_block / user
            ...
            - 입력: "나무블록은 pos2에 두고 공구는 pos1에 둬"  
            출력: wood_block tool / pos2 pos1
        ...
        self.get_logger().info("wait for client's request... ('Jarvis' wakeup word active)")
