...
class YoloInferenceNode(Node):
    def __init__(self):
        ...
        # YOLOv8 ONNX 모델 로드
        # 현재 파일 위치 기준으로 모델 파일 경로 설정
        current_dir = os.path.dirname(os.path.abspath(__file__))
        model_path = os.path.join(current_dir, 'model', 'best.onnx')
        if not os.path.exists(model_path):
            model_path = os.path.join(current_dir, 'model', 'best_1.onnx') # fallback
...
def main(args=None):
    ...
    try:
        from DSR_ROBOT2 import task_compliance_ctrl, release_compliance_ctrl, get_current_posx, set_tcp
